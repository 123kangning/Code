...
//第二次执行exec时，进入到connection.go 中的 (mc *mysqlConn) QueryContext方法，
//rows, err := mc.query(query, dargs)出错 err=invalid connection rows=nil

//github.com/go-sql-driver/mysql@v1.7.0/buffer.go中的
...
// }
// 会返回 busy buffer
//在go-sql-driver中，mysqlConn.query()方法在执行时，mysqlConn

//出现busy buffer
mysql.(*mysqlConn).interpolateParams (connection.go:208) github.com/go-sql-driver/mysql
mysql.(*mysqlConn).query (connection.go:368) github.com/go-sql-driver/mysql
mysql.(*mysqlConn).QueryContext (connection.go:505) github.com/go-sql-driver/mysql
util.CtxDriverQuery (ctxutil.go:65) github.com/seata/seata-go/pkg/datasource/sql/util
at.(*selectForUpdateExecutor).exec (select_for_update_executor.go:323) github.com/seata/seata-go/pkg/datasource/sql/exec/at
at.(*selectForUpdateExecutor).doExecContext (select_for_update_executor.go:187) github.com/seata/seata-go/pkg/datasource/sql/exec/at
at.(*selectForUpdateExecutor).ExecContext (select_for_update_executor.go:112) github.com/seata/seata-go/pkg/datasource/sql/exec/at
at.(*ATExecutor).ExecWithNamedValue (at_executor.go:76) github.com/seata/seata-go/pkg/datasource/sql/exec/at
